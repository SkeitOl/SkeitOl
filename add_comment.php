...
if(!empty($recaptcha))
{
	...
	else
	{
		...
		if(!(strlen($name)>0 && strlen($name<100)))die("<span class='text_error'>Имя пусто или слишком длинное!</span>");
		$text=htmlspecialchars($_POST['TEXT']);
		if(!(strlen($text)>0 && strlen($text<1000)))die("<span class='text_error'>Текс пуст или слишком длинный!</span>");
		$id_item=htmlspecialchars($_POST['ITEM_ID']);
		if(!($id_item!=0))die("<span class='text_error'>ID элемента пуст!</span>");
		$dt=date("Y-m-d H:i:s");
		//mysql_query("SET NAMES utf8");
		header('Content-Type: text/html; charset= utf-8');
		include_once("blocks/bd.php");
		$sql="INSERT INTO comments_articles (ID_ARTICLES,TEXT,NICK,DATE_TIME,APPROVED,IP) VALUES 
				  ('$id_item','$text','$name','$dt','0','$_SERVER[REMOTE_ADDR]')";
	 // echo$sql."<br>";
		//$result = mysql_query($sql, $db) or die('Запрос не удался: ' . mysql_error());
